...
const rolesOutputDir = path.join(outputDir, "css", "colors");

function generateColorCss() {
  return `
@import './tokens/black-alpha.css';
@import './tokens/white-alpha.css';
 ...
 */
.jds-themes:where(.light, .light-theme) {
  &,
  :root:where(:has(&[data-is-root-theme='true'])) {
    color-scheme: light;
  }
}
.jds-themes:where(.dark, .dark-theme) {
  &,
  :root:where(:has(&[data-is-root-theme='true'])) {
    color-scheme: dark;
  }
...
async function generateColorCssFiles() {
  await mkdir(rolesOutputDir, { recursive: true });
  const css = generateColorCss();

  const outFile = path.join(rolesOutputDir, `index.css`);
  await writeFileSync(outFile, css, "utf-8");
}
generateColorCssFiles();
