...
export function SEOProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const updateDocumentHead = () => {
      ...
      if (seoData.description) updateMetaTag('og:description', seoData.description, true);
      ...
      if (seoData.siteName) updateMetaTag('og:site_name', seoData.siteName, true);
      updateMetaTag('og:locale', 'ko_KR', true);

      // Twitter Card 메타데이터
      updateMetaTag('twitter:card', 'summary_large_image');
      if (seoData.title) updateMetaTag('twitter:title', seoData.title);
      if (seoData.description) updateMetaTag('twitter:description', seoData.description);
