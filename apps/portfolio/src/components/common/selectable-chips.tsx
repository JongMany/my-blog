...
function getChipClassName(isSelected: boolean, isAllButton = false): string {
  const baseStyles = "inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer border transition-colors duration-300 ease-in-out";
  
  const selectedStyles = "bg-[var(--primary)]/10 text-[var(--primary)] border-[var(--primary)]/30 font-semibold";
  
  const unselectedStyles = isAllButton
